...
def data_loader(data, train=True, batch_size = 64):
    if train: 
...
def initial_checkpoint(Model, Save_Dir, Train_data):
    ...
    if type(Save_Dir) == type(None):
        Save_Dir = './'
        print("Model checkpoint directory not specified, model will saved at ./checkpoint.pth")
    ...
    if isdir(Save_Dir):
        ...
        # Save checkpoint
        checkpointfile = os.path.join(Save_Dir, 'checkpoint.pth')
...
def main():
    global print_every, steps, data_dir
    ...
    trainloader = data_loader(train_data, train=True, batch_size=100)
    validloader = data_loader(valid_data, train=False, batch_size=50)
    testloader = data_loader(test_data, train=False, batch_size=50)
